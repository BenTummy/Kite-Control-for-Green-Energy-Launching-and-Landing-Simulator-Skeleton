...
%               includes code to generate and save a video of simulation     %
%               results                                                      %
%-----------------------------------------------------------------------------
...
% Use captured frames to create a saved movie file (Ben Tummon 24/10/2022)
KiteMov = VideoWriter ('SimulationOutput.avi');
open(KiteMov);
...
for i=1:1:length(T)
   ...
   if Ind==700 || i==1
       ...
       Ind = 1;
       % save current plot as a frame (Ben Tummon 24/10/2022)
       KiteFrame = getframe;
       % write frame to video file (Ben Tummon 25/10/2022)
       writeVideo(KiteMov, KiteFrame);
   else
       Ind = Ind+1;
   end
   pause(0.001)
